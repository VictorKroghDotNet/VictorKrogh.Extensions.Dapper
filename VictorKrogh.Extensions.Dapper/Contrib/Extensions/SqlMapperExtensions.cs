...
namespace Dapper.Contrib.Extensions
{
    ...
    public static class SqlMapperExtensions
    {
        ...
        private static List<PropertyInfo> DatabaseGeneratedPropertiesCache(Type? type)
        {
            ...
            if (DatabaseGeneratedProperties.TryGetValue(type.TypeHandle, out var pi))
            {
                return pi.ToList();
            }
        ...
        {
            if (TypeProperties.TryGetValue(type.TypeHandle, out var pi))
            {
                return pi.ToList();
            }
...
#pragma warning restore CA2211 // Non-constant fields should not be visible
        ...
        {
            if (TypeSchemaName.TryGetValue(type.TypeHandle, out var name))
            {
                return name;
            }
...
#pragma warning restore CA2211 // Non-constant fields should not be visible
        ...
        internal static string GetColumnName(PropertyInfo propertyInfo)
        {
            ...
            if (TypeColumnName.TryGetValue(propertyInfo.DeclaringType!.TypeHandle, out var columnMap))
            {
                if (columnMap.TryGetValue(propertyInfo, out var name))
                {
                    return name;
                }
            }
        ...
        {
#if NET6_0_OR_GREATER
            ArgumentNullException.ThrowIfNull(nameof(entityToInsert));
#else
            ...
            var type = typeof(T).GetCorrectType();
            var adapter = GetFormatter(connection);
            ...
            if (entityToInsert is IEnumerable entitiesToInsert)
            {
        ...
        {
#if NET6_0_OR_GREATER
            ArgumentNullException.ThrowIfNull(nameof(entityToUpdate));
#else
        ...
        {
#if NET6_0_OR_GREATER
            ArgumentNullException.ThrowIfNull(nameof(entityToDelete));
#else
...
#pragma warning restore CA2211 // Non-constant fields should not be visible
        ...
        private static class ProxyGenerator
        {
            ...
            public static T GetInterfaceProxy<T>()
            {
                ...
                if (TypeCache.TryGetValue(typeOfT, out var type))
                {
#if NET6_0_OR_GREATER
...
#if NET6_0_OR_GREATER
                return CreateInstance<T>(generatedType);
#else
...
#if NET6_0_OR_GREATER
                typeBuilder.DefineMethodOverrideNullable(currGetPropMthdBldr, getMethod);
                typeBuilder.DefineMethodOverrideNullable(currSetPropMthdBldr, setMethod);
#else
            ...
            private static void CreateProperty<T>(TypeBuilder typeBuilder, string propertyName, Type propType, MethodInfo setIsDirtyMethod, bool isIdentity)
            {
                ...
                if (isIdentity)
                {
                    ...
                    if (myConstructorInfo is null)
                    {
                        throw new Exception("Failed to retrieve constructor.");
                    }
