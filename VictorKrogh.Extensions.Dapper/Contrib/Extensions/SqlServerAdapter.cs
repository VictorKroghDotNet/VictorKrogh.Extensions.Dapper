...
namespace Dapper.Contrib.Extensions
{
    public class SqlServerAdapter : SqlAdapterBase, ISqlAdapter
    {
        ...
        {
            if (ANSIProperties.TryGetValue(type.TypeHandle, out var pi))
            {
                return pi.ToList();
            }
        ...
        private static object CreateParameterObject<T>(T entity)
        {
            ...
            foreach (var property in properties)
            {
                ...
                if (ansiAttribute is null)
                {
                    continue;
                }
                var value = property.GetValue(entity) as string;

                dynamicParameters.Add(property.Name, new DbString
                {
                    IsAnsi = true,
                    IsFixedLength = ansiAttribute.FixedLength.HasValue,
                    Length = ansiAttribute.FixedLength.GetValueOrDefault(),
                    Value = value
                });
            }
